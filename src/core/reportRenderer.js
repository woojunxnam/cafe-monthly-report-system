...
<body>
  <section class="section">
    ...
    <p>계좌별 총지출과 총입금 비교, 주요 카테고리 비교, 전체 합산 기준 master summary, 관련자금 흐름을 포함합니다.</p>
    <p><strong>총지출</strong> ${formatWon(master.totals.totalWithdrawal)} / <strong>총입금</strong> ${formatWon(master.totals.totalDeposit)}</p>
  </section>
  ...
  <section class="section">
    ...
    ${renderSummaryTable(master.rows, [
      ...
      { key: "totalWithdrawal", label: "총지출", numeric: true, render: (value) => formatWon(value) },
      { key: "totalDeposit", label: "총입금", numeric: true, render: (value) => formatWon(value) },
      { key: "netFlow", label: "순유입", numeric: true, render: (value) => formatWon(value) },
      { key: "withdrawalCount", label: "지출건수", numeric: true },
      { key: "depositCount", label: "입금건수", numeric: true }
    ])}
  </section>
    ...
    <section class="detail-box">
      ...
      <div class="table-wrap">
        <table class="detail-table">
          <thead><tr><th>날짜</th><th>시각</th><th>원본 이름</th><th>적요</th><th>금액</th></tr></thead>
          <tbody>
            ${group.rows
              .map(
                (row) =>
                  `<tr><td>${escapeHtml(row.dateKey)}</td><td>${escapeHtml(row.timeKey)}</td><td>${escapeHtml(
                    row.counterparty
                  )}</td><td>${escapeHtml(row.description)}</td><td class='num'>${formatWon(row[amountKey])}</td></tr>`
              )
              .join("")}
            <tr class="subtotal"><td colspan="4">합계</td><td class="num">${formatWon(group.total)}</td></tr>
          </tbody>
        </table>
      </div>
    </section>
...
function renderRelatedDetailBox(group) {
  return `
    <section class="detail-box">
      ...
      <div class="table-wrap">
        <table class="detail-table">
          <thead><tr><th>날짜</th><th>시각</th><th>구분</th><th>원본 이름</th><th>적요</th><th>금액</th></tr></thead>
          <tbody>
            ${group.rows
              .map((row) => {
                const amount = row.movementType === "deposit" ? row.deposit : row.withdrawal;
                const label = row.movementType === "deposit" ? "입금" : "출금";
                return `<tr><td>${escapeHtml(row.dateKey)}</td><td>${escapeHtml(row.timeKey)}</td><td>${label}</td><td>${escapeHtml(
                  row.counterparty
